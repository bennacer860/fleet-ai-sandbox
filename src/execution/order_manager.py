...
class OrderManager:
    ...
    def _persist_order(self, state: OrderState) -> None:
        if not self._persistence:
            return
        self._persistence.enqueue(
            "INSERT OR REPLACE INTO orders (order_id, strategy, token_id, slug, side, price, size, initial_status, final_status, rejection_reason, placed_at, resolved_at, signal_to_rest_ms, signal_to_fill_ms, tick_to_order_ms, time_to_expiry_s, market, best_bid, best_ask, spot_price, strike_price, proximity, spot_price_age_ms, sign_ms, post_ms, dry_run) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                state.order_id,
